            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    state['exp_avg_sq'] = torch.zeros_like(
                        p, memory_format=torch.preserve_format)
                    state['exp_avg_sq_num'] = torch.zeros_like(
                        p, memory_format=torch.preserve_format)

                    if self.kahan:
                        state['kahan'] = torch.zeros_like(
                            p, memory_format=torch.preserve_format)

                exp_avg, exp_avg_sq, exp_avg_sq_num = state[
                    'exp_avg'], state['exp_avg_sq'], state['exp_avg_sq_num']
                ...
                max_g = torch.max((1-beta1) * grad.abs(),
                                  math.sqrt(beta2)*exp_avg_sq_num)
                min_g = torch.min((1-beta1) * grad.abs(),
                                  math.sqrt(beta2)*exp_avg_sq_num)
                factor = torch.sqrt(1 + (min_g/(max_g+group['eps']))**2)
                exp_avg_sq_num -= exp_avg_sq_num
                exp_avg_sq_num += max_g * \
                    torch.sqrt(1 + (min_g/(max_g+group['eps']))**2)
                # a^2 = (beta)b^2 + (1-beta) c^2
                # a =  b times sqrt(beta + (1 - beta)* (c/b)^2)
                denom = (exp_avg_sq_num / math.sqrt(bias_correction2)
                         ).add_(group['eps'])

                step_size = group['lr'] * (1-beta1) / \
                    (math.sqrt(1-beta2) * bias_correction1)
...
class MixedPrecisionAdam(torch.optim.Adam):
    """
...
class KahanAdam(torch.optim.Adam):
